...
static DEFINE_PER_CPU(struct delayed_work, ondemandq_work);
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	schedule_delayed_work_on(cpu, &per_cpu(ondemandq_work, cpu), delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info, int cpu)
{
	...
	dbs_info->sample_type = DBS_NORMAL_SAMPLE;
	cancel_delayed_work_sync(&per_cpu(ondemandq_work, cpu));
	schedule_delayed_work_on(cpu, &per_cpu(ondemandq_work, cpu), delay);
}
...
static struct notifier_block __refdata ondemandq_cpu_notifier = {
...
static void cpu_down_work(void)
{
	int cpu;
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_sw_coordinated_cpus()) {
			register_hotcpu_notifier(&ondemandq_cpu_notifier);
	...
	case CPUFREQ_GOV_STOP:
		...
		if (!dbs_enable) {
			...
			if (dbs_sw_coordinated_cpus()) {
				...
				unregister_hotcpu_notifier(&ondemandq_cpu_notifier);
			}
		}
		break;
